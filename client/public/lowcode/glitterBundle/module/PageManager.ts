...
export class PageManager {
  public static hidePageView(id: string, del: boolean = false) {
    ...
    try {
      const index = glitter.pageConfig
        .map(data => {
          return data.id;
        })
        .indexOf(id);
  ...
  public static setHome(
    url: string,
    tag: string,
    obj: any,
    option: { animation?: AnimationConfig; backGroundColor?: string; dismiss?: () => void } = {}
  ) {
    const glitter = Glitter.glitter;
    ...
    $('#glitterPage')
      .append(`<page-box id="page${config!.id}" style="min-width: 100vw;min-height: 100vh;left: 0;top: 0;width:100vw;
background: ${config!.backGroundColor};display: none;z-index: 9999;overflow: hidden;">
    ...
    if (module) {
      ...
      const search = glitter.setSearchParam(
        glitter.removeSearchParam(glitter.window.location.search, 'page'),
        'page',
        tag
      );
      try {
  ...
  public static setLoadingView(link: string) {
    ...
    glitter
      .$('#loadingView')
      .append(
        '<iframe  src="' + link + '" style="width: 100%;height: 100%;background-color: rgba(0, 0, 0, 0.5);"></iframe>'
      );
  }
...
background: transparent;background: ${config!.backGroundColor};display: none;position: absolute;top: 0;left: 0;">
  ...
  public static innerDialog = (
    html: (gvc: GVC) => string | Promise<string>,
    tag: string,
    option: { animation?: AnimationConfig; backGroundColor?: string; dismiss?: () => void } = {}
  ) => {
    const glitter = Glitter.glitter;
    glitter.openDiaLog(
      new URL('../dialog/dialog_inner.js', import.meta.url).href,
      tag,
      {
        getView: html,
      },
