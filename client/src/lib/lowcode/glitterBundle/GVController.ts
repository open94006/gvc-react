...
export class GVC {
    ...
    public addObserver(obj: any, callback: () => void, viewBind?: string) {
        ...
        try {
            ...
            if (viewBind) {
                if (
                    map.GlitterObServerCallBack().filter(function (it: any) {
    ...
    public bindView(
        map:
            | (() => {
                  ...
                  bind: string;
                  divCreate?:
                      | { elem?: string; style?: string; class?: string; option?: { key: string; value: string }[] }
                      | (() => { elem?: string; style?: string; class?: string; option?: { key: string; value: string }[] });
                  dataList?: { obj: any; key: string }[];
                  onCreate?: () => void;
                  onInitial?: () => void;
                  onDestroy?: () => void;
              })
...
></${divCreate.elem ?? 'div'}>`;
    ...
    public addStyle(style: string) {
        ...
        if (
            !gvc.parameter.styleList.find((dd) => {
    ...
    public async addStyleLink(fs: string | string[]) {
        ...
        function add(filePath: string) {
            ...
            link.id = id;
            if (
                !gvc.parameter.styleLinks.find((dd) => {
    ...
    public addMtScript(urlArray: any[], success: () => void, error: (message: string) => void) {
        ...
        function addScript() {
            ...
            if (
                that.parameter.jsList.find((dd: any) => {
    ...
    if (
        ...
        glitter.modelJsList.push({
            ...
            create: (glitter: Glitter) => {
                init(glitter.nowPageConfig!.src, fun, (window as any).glitter as Glitter);
            },
        });
    }
