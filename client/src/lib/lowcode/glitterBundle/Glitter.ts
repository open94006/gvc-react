...
export class Glitter {
    ...
    public setPro(
        ...
        data: string = '',
        callBack: (data: {}) => void,
        option: { defineType?: any; webFunction: (data: any, callback: (data: any) => void) => any } = {
...
${!error.message ? `` : `錯誤訊息:${error.message}`}${!error.lineNumber ? `` : `錯誤行數:${!error.lineNumber}`}${!error.columnNumber ? `` : `錯誤位置:${error.columnNumber}`}\n${this.ut.dateFormat(new Date(), 'yyyy-MM-dd hh:mm:ss')}
    ...
    public setUrlParameter(tag: string, value?: string) {
        var search = value !== undefined ? this.setSearchParam(this.removeSearchParam(window.location.search, tag), tag, value) : this.removeSearchParam(window.location.search, tag);
        try {
    ...
    public setDrawer(src: string, callback: () => void) {
        ...
        if ((window as any).drawer === undefined) {
            gliter.addMtScript(
                ['glitterBundle/plugins/NaviGation.js'],
                () => {
                    callback();
                    $('#Navigation').html(src);
                },
    ...
    public addStyle(style: string) {
        ...
        if (
            !glitter.parameter.styleList.find((dd: any) => {
